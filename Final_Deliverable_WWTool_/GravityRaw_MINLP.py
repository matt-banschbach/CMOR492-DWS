...
# all the arcs that are connected to a node, which have not already been inspected in the repetition variable
def findconnectingnodes(source, arclist, repetition, arcs):
    return_list = list()
    for i in range(len(arclist)):
        for j in source:
            if (arcs[i][0] == j or arcs[i][1] == j) and (arclist[i] not in repetition) and (
                    arclist[i][::-1] not in repetition):
                return_list.append(arclist[i])
    return return_list
...
# we start an excel notebook to track the cost of each objective for each cluster for this model
...
for cluster in range(1, (n_groups + 1)):
    ...
    for i in startpoints:
        ...
        while currentpoint != end:
            ...
            if (currentpoint, to) not in arcFlow:
                arcFlow[currentpoint, to] = sum(move) + float(df.loc[df['n_id'] == currentpoint]['n_demand']) * 250 / (
    ...
    for i, j in arcFlow:
        pipeflow[i, j] = pipeflow[i, j] / 15850  # Scale factor
    ...
    # using gravity flow to drain to the next nodes after its been pushed up, to do this we created two elevations for each node
    # eIn is the elevation for a pipe coming into a node, eOut is the elevation of a pipe coming out of a node
    ...
    m.addConstrs((gp.quicksum(arc_sizes[i, j, k] for k in pipesize) == 1 for i, j in pipeflow.keys()),
                 "single size chosen")
    # in gurobi you cannot multiply three variables or set variables to powers so you have to create general constraints and auxiliary variables
